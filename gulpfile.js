...
const PATHS = {
  ...
  image: {
    ...
    dest: `${staticBase}/assets/images/`,
    destwp: `${wpThemesBase}/assets/images/`,
  },
  sprite: {
...
const sassFunc = (done) => {
  ...
  return src(PATHS.styles.src, { sourcemaps: isCssMap, base: null })
    ...
    // 納品用に min 版（style.min.css / fv.min.css）も出力する
...
const jsFunc = () => {
  ...
  // ファーストビュー用JS（fv.js）は結合せず単体で出力する
  src([PATHS.js.fvSrc], { sourcemaps: isJsMap, allowEmpty: true })
    .pipe(plumber({ errorHandler: errorHandler }))
  ...
  return src([PATHS.js.src], { sourcemaps: isJsMap })
    .pipe(plumber({ errorHandler: errorHandler }))
    .pipe(order(["common.js", "*.js"], { base: "./src/js/" }))
